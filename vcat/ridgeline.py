...
class Ridgeline(object):
    ...
    def get_ridgeline_luca(self,image_data,noise,error,pixel_size,beam,X_ra,Y_dec,counterjet=False,angle_for_slices=0,cut_radial=5.0,
        ...
        for x in range(0, i):
            ...
            logger.info("The FWHM (convolved) is = " + str(FWHM))
            chi_sq = 0.0
    ...
    def plot(self,mode="",savefig="",fit=True,start_fit=5,skip_fit=3,avg_fit=3,fig="",ax="",show=True):
        ...
        if mode=="open_angle":
            ...
            if fit==True:
                ...
                perr = np.sqrt(np.diag(pcov))
                logger.info('Fit values (a*x**b) with a the first term and b the second -- First method')
                logger.info(popt)
                logger.info(perr)
                ...
                popt, pcov = curve_fit(func, dist_fit, width_fit, sigma=width_err_fit)
                perr = np.sqrt(np.diag(pcov))
                logger.info('Fit values (a*x**b) with a the first term and b the second -- Second method')
                logger.info(popt)
                logger.info(perr)

                logger.info('Valori fit media')
                logger.info(dist_fit)
                logger.info(width_fit)
